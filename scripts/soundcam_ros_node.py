...
class SoundcamROS(object):
    ...
    def devStreamPublishing(self, streamType, devStrObj:pf.FakeWebcam=None):
    ...
    def videoPublishing(self, pub:rospy.Publisher, streamType):
        pub_rate = 30.0
    ...
    def _takeSnapshot(self, streamType=SoundcamServiceRequest.OVERLAY_STREAM, 
                      ...
                      pose3dInfo:ROSLayerUtils.Pose3dInfo=None):
        isActPoint = True
        media = list()
        frame_data = list()
        if(tileInfo is None):
        ...
        if((wpInfo is not None) and (len(media) > 0)):
            self.utils.addMetaData(
                ...
                pose3dInfo=ROSLayerUtils.Pose3dInfo(*pose3dInfo),
    ...
    def executeCB(self, goal: SoundcamGoal):
        ...
        if(can_proceed):
            ...
            self.recordTrigger.clear() #deactivate record triggers
            ...
            pose3d:ROSLayerUtils.Pose3dInfo = ROSLayerUtils.Pose3dInfo(0.0, 0.0, 0.0)
            with self.tileLock:
            ...
            #run while loop
            rate = rospy.Rate(15)
            result = False #results from this action running
            cnt = 1
            ...
            while(not rospy.is_shutdown()):
                if((recordTime <= self.cfg['min_record_time']) and (numCaptures > 0)): #Take Snapshots
                    ...
                    if(not self.recordTrigger.is_set()): #start recording
                ...
                if(self.missionData.id != 0): #only perform auto-detection when explicitly required
                    if(self.autoDetect):
                        ...
                        if(self.signalInfo.pre_activation and 
                           ...
                           not self.recordTrigger.is_set() and
                        ...
                        if(self.signalInfo.detection):
                            ...
                            if(self.recordTrigger.is_set() and ((time.time()-record_start_t)) >= (self.curCaptureTime * self._f_mul)):
